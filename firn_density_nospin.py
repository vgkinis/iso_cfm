...
class FirnDensityNoSpin:
    ...
    def __init__(self, configName):
        ...
        Tsf = interpolate.interp1d(input_year_temp,input_temp,'linear',fill_value='extrapolate')
        ...
        bsf = interpolate.interp1d(input_year_bdot,input_bdot,'linear',fill_value='extrapolate')
        ...
        if 'iso_sigma18' in self.output_list:
            self.iso_sigma18_out = np.zeros((TWlen+1, len(self.dz)+1), dtype = 'float32')
            self.iso_sigma18_out[0,:] = np.append(self.modeltime[0], self.iso_sigma['18'])
    ...
    def time_evolve(self):
        ...
        for iii in range(self.stp):
            ...
            if mtime in self.TWrite:
                ...
                if 'iso_sigmaD' in self.output_list:
                    self.iso_sigmaD_out[self.WTracker,:] = np.append(mtime_plus1, self.iso_sigma['D'])
                if 'iso_sigma18' in self.output_list:
